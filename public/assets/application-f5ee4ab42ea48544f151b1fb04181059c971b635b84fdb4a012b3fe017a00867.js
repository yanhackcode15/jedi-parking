...
var address = '<%= @address %>';
var map, watchId, infowindow, myMarker, marker, meterMarker_list, locations;
...
.done(function (data) {
	...
	// for(var i=0; i<500; i++){
		var meter=[];
		meter[0]=data[i]['address'];
		meter[1]=data[i]['latitude'];
		meter[2]=data[i]['longitude'];
		meter[3]=data[i]['meter_id'];
		meter[4]=data[i]['event_type'];
		meter[5]=data[i]['event_time'];
		locations[i] = meter;   
	}

	var optionsShowMyMarker = {
		enableHighAccuracy: false,
		timeout: 5000,
		maximumAge: 0
	};

	watchId = navigator.geolocation.watchPosition(showMyMarker, errorShowMyMarker, optionsShowMyMarker);
...
function showMeterMarkers() {
	...
	if (meterMarker_list.length!=0) {
		for (var m in meterMarker_list){
			marker.setMap(null);
		}
	}
	//add new markers that are within the bound
...
function showMyMarker(pos){
	...
	myMarker.setMap(map);
	myMarker.setOptions({icon: geoIcon});
}

function errorShowMyMarker(){
	...
	return new Promise(function (resolve, reject) {
		...
		geocoder.geocode({ address: address }, function (results, status) {
			if (status !== google.maps.GeocoderStatus.OK) {
				reject('The address is either invalid or is not specific enough.');
			}
